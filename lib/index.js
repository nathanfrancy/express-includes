...
function IncludeMiddleware(options) {
  ...
  if (self.minified && self.dirName) {
    ...
    self.dirs = getDirectories(self.dirName);
    self.dirs.forEach(function(el, i, arr) {
      self.dirs[i] = self.dirName + self.dirs[i];

      var firstLevelDirs = getDirectories(self.dirs[i]);
      firstLevelDirs.forEach(function(firstLevelEl, firstLevelIndex, firstLevelArray) {
        self.dirs.push(self.dirs[i] + '/' + firstLevelDirs[firstLevelIndex]);
      });
    });
